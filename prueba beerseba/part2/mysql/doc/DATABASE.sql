...
SELECT  tipo_producto.tipo,
    producto.nombre,
    producto_talle.talle,
    producto_talle.stock, 
    producto_talle.descripción
FROM producto  
INNER JOIN tipo_producto  ON tipo_producto.id = producto.id_tipo_producto
INNER JOIN producto_talle  ON producto_talle.id_producto = producto.id;
...
SELECT compra.fechahora, 
        ...
        compra.estado
FROM compra  
INNER JOIN USUARIOS  ON usuarios.id = compra.id_usuario 
INNER JOIN Compra_Detalle ON compra.id = compra_detalle.id_compra
INNER JOIN Producto_talle ON compra_detalle.Id_producto_talle = producto_talle.id
INNER JOIN producto  ON producto.id = producto_talle.id_producto;
